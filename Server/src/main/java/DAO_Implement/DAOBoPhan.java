...
public class DAOBoPhan extends UnicastRemoteObject implements IDAOBoPhan {
    ...
    public ArrayList<BoPhan> getAll() throws RemoteException {
        ...
        try {
            ArrayList<BoPhan> resultList = (ArrayList<BoPhan>) em.createNativeQuery("SELECT * FROM BoPhan",
                    BoPhan.class).getResultList();
            return resultList;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public ArrayList<BoPhan> filter(String prefix) throws RemoteException {
        ...
        try {
            ArrayList<BoPhan> resultList = (ArrayList<BoPhan>) em.createNativeQuery("SELECT * FROM BoPhan WHERE maBP LIKE ? OR tenBP LIKE ?",
                    BoPhan.class).setParameter(1, prefix + "%").setParameter(2, prefix + "%").getResultList();
            return resultList;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
